...
# Process-wide cap on concurrent OpenAlex calls so many parallel searches don't
# burst into 429s; combined with the @retry backoff this keeps us in the polite pool.
...
async def find_author(
    ...
    r = await _throttled(client.get(f"{BASE}/authors", params=params, timeout=20))
    r.raise_for_status()
...
async def find_institution(client: httpx.AsyncClient, name: str) -> dict | None:
    params = {"search": name, "per_page": 1, "mailto": settings.openalex_mailto}
    r = await _throttled(client.get(f"{BASE}/institutions", params=params, timeout=20))
    r.raise_for_status()
    results = r.json().get("results", [])
    return results[0] if results else None
...
async def works_by_field(
    ...
    r = await _throttled(client.get(f"{BASE}/works", params=params, timeout=20))
    r.raise_for_status()
    agg: dict[str, dict] = {}
...
def _tail(oid: str | None) -> str:
    return (oid or "").rstrip("/").rsplit("/", 1)[-1]


@retry(stop=stop_after_attempt(3), wait=wait_exponential(min=1, max=8))
async def _recent_inst_work_count(
    client: httpx.AsyncClient, author_id: str, institution_id: str, since: str
) -> int:
    """Count this author's own works since `since` where their authorship lists the institution."""
    rid, iid = _tail(author_id), _tail(institution_id)
    r = await _throttled(client.get(f"{BASE}/works", params={
        "filter": f"author.id:{rid},from_publication_date:{since}",
        "select": "authorships", "per_page": 50, "mailto": settings.openalex_mailto,
    }, timeout=20))
    r.raise_for_status()
    count = 0
    for work in r.json().get("results", []):
        for a in work.get("authorships", []):
            if _tail((a.get("author") or {}).get("id")) == rid:
                if iid in {_tail(i.get("id")) for i in a.get("institutions", [])}:
                    count += 1
                break
    return count


async def recent_active_author_ids(
    client: httpx.AsyncClient, institution_id: str, author_ids: list[str],
    since: str, min_works: int = 2,
) -> set[str]:
    """Author ids still active at the institution: >= min_works whose own authorship
    lists it since `since` (drops affiliation-lag/departed people). Fails open per author."""
    iid = _tail(institution_id)

    async def check(aid: str) -> str | None:
        key = f"active:{iid}:{_tail(aid)}:{since}"
        cached = await cache.get_json(key)
        if cached is not None:
            return aid if cached else None
        try:
            active = await _recent_inst_work_count(client, aid, institution_id, since) >= min_works
        except Exception:
            return aid  # don't drop a candidate on a transient error
        await cache.set_json(key, active, settings.cache_recency_ttl)
        return aid if active else None
    return {a for a in await asyncio.gather(*(check(a) for a in author_ids)) if a}
...
async def author_works(
    client: httpx.AsyncClient, author_id: str, limit: int = 10, since: str | None = None
) -> list[dict]:
    """Most-cited works for an author; `since` restricts to recent years so the
    research corpus reflects current work, not decades-old classics."""
    filt = f"author.id:{_tail(author_id)}"
    if since:
